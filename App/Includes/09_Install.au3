...
Func Au3RunFix($p_Num = 0)
	...
	$g_LogFile = $g_LogDir & '\EE-Debug-Installation.log'
	; No longer used
...
Func Au3Install($p_Num = 0, $p_Debug = 0)
	...
	If $p_Debug = 1 Then
		...
		Next
		_PrintDebug('Installation array printed to log App\Logs\EE-Debug.log',1)
		Exit
	EndIf
...
; ---------------------------------------------------------------------------------------------
	...
	GUICtrlSetState($g_UI_Button[0][3], $Gui_ENABLE)
	$g_LogFile = _ResetInstall() & '\EE-Debug-Installation.log'
	If StringRegExp($g_Flags[14], 'BWP|BWS') Then
...
Func _Install_CompressLog()
	Dim $LastLine
	$File = $g_LogDir & '\EE-Debug-Installation.log'
	$Handle = FileOpen($g_LogDir & '\EE-Debug.log', 2)
	...
	$Handle = Run('"' & $7za & '" a "' & $g_GameDir & '\EE-Debug.7z" "' & $g_LogDir & '\EE-Debug.log"', $g_ProgDir, @SW_HIDE, 8)
	Local $Return
	While 1
...
Func _Install_ModifyForGroupInstall($p_Array, $p_Debug=0)
	...
	If $p_Debug = 1 Then
		...
		Next
		_PrintDebug('_Install_ModifyForGroupInstall finished - check App\Logs\EE-Debug.log',1)
		Exit
	EndIf
	Return $NArray
