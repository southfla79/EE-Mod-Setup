...
Func _Misc_Set_GConfDir($p_GameType)
	...
	If StringRegExp($p_GameType, 'BG[1-2]EE') Then
		$g_ConnectionsConfDir = $g_ProgDir&'\Config\EET'; make BG1EE and BG2EE use EET Game.ini for Connections rules
	Else
...
Func _Misc_SwitchGUIToInstallMethod()
	...
	Next
	If $found = 0 Then
		_PrintDebug('There is a problem with the internal configuration files. The App\Config\User.ini indicates that your current game type is "'&$g_Flags[14]&'" but no matching configuration was found. Please try restarting tool using "update" to get the latest files; if that fails, please report this problem on one of the support forums.', 1)
		Exit
	EndIf
	_Misc_Set_GConfDir($g_GameList[$g][0])
