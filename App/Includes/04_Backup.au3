...
Func Au3CleanInst($p_Num = 0, $p_Tab = 6) ;1=first timer, 2=backup, 3=restore
	...
	Global $g_LogFile = $g_LogDir & '\EE-Debug-Backup.log'
	$g_Flags[0]=1
...
Func _Backup_Create($p_Game, $p_Message)
	...
	For $f=1 to $Files[0]
		...
		If StringRegExp($Files[$f], '(?i)\A(Big World Backup|Big World Downloads|App|cache|cd\d|data|ereg|mplayer|script compiler|debugs)\z') Then ContinueLoop; don't copy useless or untouched folders
		If $IsDir Then
	...
	Local $Fix[6] = ['portraits', 'save', 'mpsave', 'App', 'Big World Backup', 'Big World Downloads']
	For $f in $Fix
		_IniWrite($Section, $f, 1)
	Next
	_IniWrite($Section, 'Start BWS-EE Setup.vbs', 0)
	_IniWrite($Section, 'Update BWS-EE Setup.vbs', 0)
...
; ---------------------------------------------------------------------------------------------
	...
	For $b=1 to $BakFiles[0]
		If StringRegExp($BakFiles[$b], '(?i)\A(CD\d|Big World Downloads|Big World Backup|App|Portraits)\z') Then ContinueLoop; do >>not<< remove useful or essential folders
...
; ---------------------------------------------------------------------------------------------
		...
		For $f=1 to $Files[0]
			If _IniRead($ReadSection, $Files[$f], -1) <> -1 Then ContinueLoop; includes portraits, save, mpsave, App + its vbs, Big World Backup, Big World Downloads
			$FileList[0][0] +=1
...
; ---------------------------------------------------------------------------------------------
	...
	For $b=1 to $BakFiles[0]
		If StringRegExp($BakFiles[$b], '(?i)\A(Big World Downloads|Big World Backup|App|mpsave|Portraits|save|BWS_Backup.ini)\z') Then ContinueLoop; do >>not<< copy old content
