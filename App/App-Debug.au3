...
If $IsCrashed = 1 Then
	If @extended = 0 Or FileExists ($g_DebugFile) Then
		If FileExists($g_DebugFile) Then 
			...
			FileClose($Handle)
		EndIf
		MsgBox(48, 'EE Game Setup - '&_GetSTR('T1'), _GetSTR('L1'))
		ShellExecute($g_TraceFile)
		Exit
	EndIf	
	$Answer=MsgBox(3+32, 'EE Game Setup - '&_GetSTR('T1'), _GetSTR('L2'))
	If $Answer=6 Then
		Run(@ComSpec & ' /c AutoIt3.exe /ErrorStdOut Traced.au3 | AutoIt3.exe "App-Debug.au3"', @ScriptDir, @SW_HIDE)
...
; ---------------------------------------------------------------------------------------------	
		...
		ElseIf $Num = '00' Then
			$Line=_FileReadLine(@ScriptDir&'\App.au3', StringRegExpReplace($String[$s], '.*\s', ''))
...
Func Trace ()
	...
	_AddTrace(@ScriptDir&'\App.au3', @ScriptDir&'\Traced.au3', '00')
